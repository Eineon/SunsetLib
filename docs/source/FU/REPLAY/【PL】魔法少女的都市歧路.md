...
当警报响彻银色穹顶时，稚嫩的少女们携手奔向数据之海深处。夜莺感受着掌心传来的温度，那个总爱在虚拟空间护理花草的孩子，此刻瞳孔中却流转着不属于人类的虹光。

「要活下去。」随着这句诅咒般的话语，那个孩子将她推进锈蚀的通风管道，植入后颈的芯片已经开始过载发烫。<br>
追捕者的脚步声混着金属关节摩擦声逼近，夜莺透过栅格看见那个孩子被三根碳纤维探针刺穿锁骨，女孩最后的表情却像是在花园里修剪枝条般平静。

流浪在霓虹沼泽的灵能者，总会在雨季听见往生者的呢喃。那双透明的手会为她拂去睫毛上的雨珠，会在她凝视深渊时轻轻遮住她的眼睛，就像多年前的虚拟空间里，那个孩子捂住她窥见真相的双眸那样。
...
**体重**：47KG<br>
**基因谱系**：人类基准基因组<br>
**隶属机构**：前S公司第VII期认知强化项目

**项目代号**：Mnemonic Weaver<br>
**运作时间**：新世纪982-996<br>
**研究目标**：开发基于海马体神经可塑性调控的记忆编辑技术<br>
**技术路径**：

- 通过病毒载体增强杏仁核-前额叶皮层神经联结
- 利用经颅磁刺激诱导情景记忆具象化
- 构建人工记忆迷宫进行认知行为调控
...
- 05:32 实验舱电力系统遭人为破坏
- 05:47 NX-0912经由通风管道逃离第三研究区
- 05:53 NX-0915主动触发神经过载，致使12名追捕人员陷入强直性昏厥
...
- 每当你在冲突场景之外进行**有意义的检定**并取得成功时，你会恢复1点灵知，具体由GM判断。
...
- 你可以在自己进行对抗检定、开放检定或【阻碍】检定时的其中1项属性替换为「WLP」。
- 每场景1次，你可以基于当前情景询问GM任意1个问题。GM可以选择不回答；若是如此，则不会消耗使用次数。
- 你不需要动手，就可以缓慢移动远处的物品，也可以像用手一样做出按下、抓起、扭动等动作。
- 你可以将记忆中的物品投影到现实中，持续至场景结束。<br>
  你最多只能同时维持1个这样的物品，并且必须对其具有深入的了解，具体由GM判断。
:::
...
每回合1次，当敌人陷入你制造的状态效果，或者你对敌人执行【研究】获得到达「完整」程度的信息时，你可以使用本天赋，使用装备的武器进行一次**自由攻击**。<br>
这次攻击将会对抗目标物防与魔防中的较低者。在计算伤害时，将HR视为0。<br>
你无法在已经进行过自由攻击回合使用本天赋，反之亦然。<br>
...
- 创造时选择任意1种武器类别。如果选择**弓**、**火器**或**投掷**，则视为远程武器；否则视为近战武器。
- 创造时选择双手或单手。如果选择双手，则将「HR+当前WLP骰尺寸」作为伤害；否则将「HR+当前WLP骰尺寸/2」作为伤害。伤害属性为物理。
- 以「【WLP+WLP】+目标状态效果数量」作为命中检定。

幻影武器会悬浮在你身边，以你为中心移动。当你的主手或副手装备物品时，所有幻影武器都会立即消散。<br>
当你能够**使用装备的武器进行攻击**时，可以改为令1件幻影武器进行1次攻击。

- 你可以使用相同类别的单手幻影武器进行**双武器战斗**。
- 你可以将近战幻影武器发射出去，从而进行远程攻击。离开你身边的幻影武器将在回合结束时消散。
- 每轮1次，你可以对1名被幻影武器击中的生物造成［该生物状态效果数量\*2］点额外伤害。
- 每次使用幻影武器进行攻击前，你可以根据攻击目标当前承受的状态效果，消耗5点MP改变本次攻击的伤害属性。<br>
  如果存在多名攻击目标，或者攻击目标同时承受多种状态效果，则可以从所有状态的对应伤害属性中进行选择。
  + **缓慢**：风或毒
  + **眩晕**：光或土
  + **虚弱**：暗或冰
  + **动摇**：电或火

你还可以通过消耗幻影武器执行【荒疫诅咒】【锥身诅咒】【心灵堡垒】。
:::{admonition} 【荒疫诅咒】
每轮1次，当你对1名承受状态效果的生物造成伤害时，可以消耗1件幻影武器，给予以下诅咒之一。<br>
这些诅咒视为状态效果。一个场景内，同一名生物最多只会受到1次相同的诅咒。

- **食尘诅咒**：如果生物处于**缓慢**状态，则她需要使用2次【攻击】动作才能完成1次近战攻击。<br>
  承受该诅咒的生物，DEX骰尺寸-1(最低D4)，并且无法免疫和解除**缓慢**状态。
- **谵妄诅咒**：如果生物处于**眩晕**状态，则她使用仅将自己或盟友作为目标的动作时，必须进行1次DL10的【INS+WLP】检定。失败则动作无效。<br>
  承受该诅咒的生物，INS骰尺寸-1(最低D4)，并且无法免疫和解除**眩晕**状态。
- **幼化诅咒**：如果生物处于**虚弱**状态，则她通过武器攻击预定造成的伤害降低［状态效果数量］点。<br>
  承受该诅咒的生物，MIG骰尺寸-1(最低D4)，并且无法免疫和解除**虚弱**状态。
- **涣散诅咒**：如果生物处于**动摇**状态，则她通过咒语预定造成的HP／MP损失和恢复效果降低［状态效果数量］点。<br>
  承受该诅咒的生物，WLP骰尺寸-1(最低D4)，并且无法免疫和解除**动摇**状态。
:::
:::{admonition} 【锥身诅咒】
...
- 如果对该生物进行命中检定，只要2个骰子显示数字都为6及以上，并且相差不超过遮断点数，即视为**大成功**。
:::
