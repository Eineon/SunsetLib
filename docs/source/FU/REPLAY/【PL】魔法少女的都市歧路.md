...
这次攻击将会对抗目标物防与魔防中的较低者。在计算伤害时，将HR视为0。你无法在已经进行过自由攻击回合使用本天赋，反之亦然。<br>
...
- **谵妄诅咒**：如果生物处于**眩晕**状态，则她使用仅将自己或盟友作为目标的动作时，必须进行1次DL10的【INS+WLP】检定。如果检定失败，动作将被无效。
  承受该诅咒的生物，INS骰尺寸-1(最低D4)，无法免疫和解除眩晕状态。
