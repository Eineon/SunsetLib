...
- **伤害骰级别**：伤害骰的级别按以下顺序排列：1D2 (1级) --> 1D4 (2级) \--> 1D6 (3级) \--> 2D4 (4级) \--> 2D5 (5级) ... \--> 2D20 (20级)。改变伤害骰级别的效果只会影响基础伤害骰。
- **骰子尺寸**：伤害骰之外的骰子尺寸会按以下顺序排列：D2 \--> D4 \--> D6 \--> D8 ... \--> D20。只有改变伤害骰级别的效果能够改变伤害骰尺寸。
- **熟练骰**：尺寸为［D[PB+2]］的骰子。
...
##### 对决
*战斗风格专长（先决：APR2+）* \
当你单手持用一把近战武器且没有持用其他武器时，你的先攻检定获得1点支援，加入该武器伤害中的攻击属性调整值 (若有) 增加50%。

##### 巨武器战斗
*战斗风格专长（先决：APR2+，力量13+）* \
当你共持拥有「双手」或「多用」词条的武器时，该武器通过**攻击姿态**获得的伤害骰级别提升增加50%。 \
每轮你都可以重掷最多［STR］枚该武器的基础伤害骰，并使用较高的结果。

##### 拦截
*战斗风格专长（先决：APR2+）* \
当你可见的生物发动攻击命中你的威胁范围内的生物时，你可以消耗1次借机攻击，使此次攻击造成的伤害减轻「2枚熟练骰」点。 \
...
##### 徒手战斗
*战斗风格专长（先决：APR2+）* \
你的徒手打击伤害骰提升至1D6。如果发动徒手打击时没有持用其他武器或盾牌，则再提升1级伤害骰级别。 \
...
- **响应法术**：当一个生物因移动引发你的借机攻击时，可以改为消耗1次借机攻击来对这个生物执行1个能够造成伤害的【魔法】动作。 \
  该【魔法】动作只会对触发本增益的生物造成伤害，即便该动作通常能够影响更多对象。 \
...
- **致命灵巧**：执行【攻击】动作・发动借机攻击时，如果使用拥有「轻型」词条的武器，将其伤害骰提升1级。副手攻击不会获得本增益。
...
如果选择简易武器，则其伤害骰级别获得1点增强加值。

##### 机敏
*战斗风格专长（先决：APR2+）* \
如果可以将DEX加在AC上，你的AC获得1点内在加值，所有速度获得2格内在加值。 \
如果在1回合内直线移动至少2格，本专长为AC提供的内在加值提升至2点，持续至你的下个回合开始。

##### 战斗反射
*战斗风格专长（先决：APR2+，敏捷13+）* \
你每轮可以额外发动［DEX］次借机攻击，但针对同一触发项依然只能发动1次。
