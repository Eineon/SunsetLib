...
#### 反击
当你被可见生物选为攻击目标时，可以在攻击者投掷攻击检定后、DM宣布结果前，使用当前持用的1把武器对其发动1次攻击检定承受2点减值的攻击作为反击。 \
反击不会挑战攻击者的AC，而是将攻击判定值作为对抗对方攻击的目标值。 \
即便攻击者不在武器射程内，你也可以进行反击。如果攻击者的攻击失手，则代表反击成功。 \
反击成功时，若攻击者身处反击武器的威胁范围内，你可以令反击命中攻击者并对其造成伤害。 \
如果攻击者的攻击判定值比你低10点或更多，命中攻击者的反击也会形成重击。

你不能在无法发动借机攻击时进行反击，并且针对每次攻击只能进行1次反击。反击不会选择攻击目标，因此无法反击反击。 \
...
- 原本的专长对移动速度的提升视为内在加值。
- 移除专长：箭术、投掷武器战斗、强健身心、中甲大师、重甲大师、双持客、粉碎者、穿刺者、劈砍者
...
强豁免的等级加值等于［职业等级÷2］，弱豁免的等级加值等于［(职业等级+2)÷3］。 \
...
##### 2级：动作如潮
使用本特性的回合，你可以进行2次全力攻击。第二次全力攻击无需使用移动动作，但在决定其效果时，视APR比实际低6级。
