...
- **增强加值**：增强加值不会互相叠加，也不会与魔法物品的加值叠加。
- **伤害骰尺寸**：伤害骰的尺寸会按以下顺序提升：D4 --> D6 --> D8 --> D10 --> D12。提升伤害骰的效果不会叠加。
...
- 如果在一次属性检定中同时拥有技能与工具的熟练，则将PB提升2点。
...
##### 要害打击
*通用专长（先决：APR5+）* \
使用【技艺】动作发动1次武器攻击，命中则额外造成［1[W]］点武器伤害，并且因**攻击姿态**而获得的伤害加值增加原本的50%。
...
#### 双持攻击
你在自己的回合执行【攻击】动作时，可以在发动攻击之前使用附赠动作进行双持攻击，令这次【攻击】能够以副手额外发动1次徒手・武器攻击。
...
以1个标准动作与移动动作，你可以对1格内可见的1名濒死生物进行处决，发动1次徒手・武器攻击。
- 进行处决会引发其他生物的借机攻击。
- 处决攻击会自动命中并形成重击。
...
#### 多重攻击
当你的APR到达5/11/17级后，在自己的回合执行【攻击】动作时，可以在发动攻击之前使用移动动作进行多重攻击，令这次【攻击】能够额外发动1/2/3次徒手・武器攻击，不可叠加。 \
...
- **攻击姿态**：使你的所有徒手・武器攻击检定承受X点减值，令每次徒手・武器攻击命中时造成的伤害获得2X点加值。X最高为［PB÷2］。
