...
- 要求豁免，并且不使用职业・法术DC的效果，其DC变为［10+PB+属性调整值+人物等级÷2］。
...
- **显能动作**：【显能】是一种新的标准动作选项。
- **属性提升**：选择2项属性+1，或选择1项属性+2。如果尝试令属性超过20，这次**属性提升**只会单独令该属性+1，最高30。
...
- **伤害骰尺寸**：伤害骰的尺寸会按以下顺序提升：1D4 --> 2D3 --> 2D4 ... 2D19 --> 2D20。提升伤害骰尺寸的效果不会叠加。
...
##### 徒手战斗
*战斗风格专长（先决：战斗风格特性）* \
你的空手可以同时视为持用具有「轻型，灵巧」词条的武器。 \
你的徒手打击同时视为武器攻击，并且可以将伤害骰提升至1D6。如果发动徒手打击时没有持用其他武器或盾牌，则可以将伤害骰提升至1D8。 \
每当你的回合开始时，你还可以对一个受擒于你的生物造成［1[W]］点钝击伤害。
...
*典范专长（先决：SPR1+）* \
你获得以下增益： 
- **专注**：你为维持专注所做的体质豁免具有优势。
- **响应法术**：当一个生物因离开你的触及范围而引发你的借机攻击时，你能够改为消耗1次借机攻击次数来对这个生物执行1个能够造成伤害的【魔法】动作。 \
...
##### 即兴发挥
*起源专长* \
进行不具熟练的属性检定时，你依然会获得1点加值。4级起，变为2点加值；6级起，变为［人物等级÷2］点加值。
...
- **灵活双持**：你可以使用附赠动作以副手发动1次徒手・武器攻击，如同进行双持攻击。
...
##### 要害打击
*通用专长（先决：APR5+）* \
使用【显能】动作发动1次武器攻击，命中则额外造成［1[W]］点武器伤害，并且因**攻击姿态**而获得的伤害加值增加原本的50%。

##### 精通要害打击
*通用专长（先决：APR11+，要害打击）* \
使用【要害打击】专长时，额外伤害提升至［2[W]］，因**攻击姿态**而获得的伤害加值再增加原本的50%。 \
APR17级起，额外伤害提升至［3[W]］，伤害加值再增加原本的100%。
...
##### 法术战斗
*战斗风格专长（先决：驭法攻击）* \
当你通过【驭法攻击】发动额外攻击时，可以加入攻击属性调整值 (若原本不行)。
...
#### 武器伤害骰
根据下表替换大于1D4的徒手・武器伤害骰，以及使用武艺骰代替徒手・武器攻击伤害时的伤害骰。
...
重击不再翻倍伤害骰，而是额外造成2D8点重击伤害，伤害类型与攻击相同。 \
如果一个效果原本会扩大你的重击范围，则每增加1点重击范围，就将重击伤害提升2D8点，不可叠加。

#### 借机攻击
借机攻击每轮可以发动1次，不再占用反应。会让你无法执行反应的效果，同样会让你无法发动借机攻击。 \
只要生物移动离开你的触及范围内的方格，就会引发你的借机攻击。

#### 双持攻击
每回合1次，你在执行【攻击】动作时，可以在发动攻击前使用附赠动作进行双持攻击，令这次【攻击】能够以副手额外发动1次徒手・武器攻击。
...
若你在敌人的触及范围内翻滚，则需要进行1次敏捷 (特技) 检定，挑战敌人的反射SC，成功则不会因移动而引发它的借机攻击。 \
你也可以进行相同的敏捷 (特技) 检定，来翻滚穿越其他生物占据的空间。 \
每进行1次翻滚，你接下来在本回合进行的所有翻滚都需要额外消耗1格移动力。

#### 濒死
剩余HP为上限的$\frac{1}{4}$或更低的生物将处于濒死。命中濒死生物的徒手・武器攻击伤害提升［1[W]］点。 \
以1个整轮动作，你可以对1格内可见的1名濒死生物进行处决，发动1次徒手・武器攻击。
- 进行处决会引发其他生物的借机攻击。
- 处决攻击会自动命中并形成重击。
- 如果处决未能将目标的HP降至0，则其必须进行1次死亡豁免，DC的关键属性为你的攻击属性。豁免失败则HP降至0。


### 法术

#### 专注
专注豁免改为单独的一种豁免检定，其豁免调整值为［PB+CON］，再加上等于［APR÷2］与［SRP÷2］中较高一方的阶层加值。
...
## 职业
- 你在1级时的HP上限变为［PB+体质值+职业生命骰尺寸］点。升级时取满你的职业生命骰。
- 所有职业的起始装备统一为10把金币。
...
拥有熟练的徒手・武器攻击检定会获得［APR÷2］点阶层加值。 \
高阶APR为［职业等级］级，中阶APR为［⌈职业等级×$\frac{3}{4}$⌉］级，低阶APR为［⌈职业等级÷2⌉］级。 \
如果进行兼职，分别计算每个职业的APR并加在一起，最后将小数向上取整。

#### 多重攻击
当你的APR到达5/11/17级后，每回合1次，你在执行【攻击】动作时，可以在发动攻击前使用移动动作进行多重攻击，令这次【攻击】能够额外发动1/2/3次徒手・武器攻击。 \
...
强豁免的阶层加值等于［等级÷2］，弱豁免的阶层加值等于［⌈等级÷3⌉］。 \
如果进行兼职，分别计算每个职业的豁免阶层加值并加在一起，最后将小数向上取整。 \
只要你拥有豁免倾向为强的职业，该基础豁免获得的阶层加值便会提升2点。

#### 豁免等级
基础豁免的（豁免等级:Saving Class）等于［10+对应豁免总调整值］。SC有时会代替AC作为攻击检定的目标值。 \
如果你的基础豁免拥有优势，其SC也会获得5点加值；若有劣势，则会承受5点减值。
...
20级起，选择1项属性，再选择A或B：(A) 所选属性+10，最高18；(B) 所选属性+2，最高30。
...
*替换【灼净亡灵】特性* \
你获得以下增益。
- **璀璨火花**：当你使用**神圣火花**时，可以消耗X点MP，选择6格内X名你可见的生物作为该引导神力的目标，并分别决定每名目标受到的效果。
...
- **月光愈合**：结社形态期间，你可以使用附赠动作消耗X点MP，将自身的临时生命提升［5X］点，最高提升至［职业等级×5］点。X最大为你的职业等级。
- **无羁野性**：你无法在结社形态下持用武器・盾牌。在结社形态下使用徒手打击时，将德鲁伊APR提升1阶，并将徒手打击视为魔法攻击，可以在徒手打击的攻击・伤害检定中使用WIS代替STR。
...
- **天生武器**：你获得【徒手战斗】作为奖励专长，并且徒手打击的触及范围提升至10尺。
- **凶暴形态**：10级起，当你受到钝击、穿刺或挥砍伤害时，可以使用反应消耗2点MP获得这三种伤害的抗性，持续至你的下个回合开始。
:::
...
- 你获得【徒手战斗】作为奖励专长。当你使用武艺骰代替徒手打击原本的伤害时，提升1级伤害骰尺寸。
- 你在自己的回合发动武僧武器攻击时，可以在掷骰前额外消耗X次武器攻击次数，使本次攻击命中时额外造成［X[W]］点武器伤害。这些攻击次数不必来自同一动作，但必须发生在同一回合。
...
当你使用灵巧或远程武器发动的攻击命中时，若本次攻击检定具有优势，或攻击目标被你・盟友夹击，则可以通过诡计骰实现下列诡计选项。 \
...
- **淬毒**：5级起，可以消耗X (最低1) 枚诡计骰，使目标在X轮内陷入中毒状态。 \
  重复施加本诡计将会延长中毒的持续时间。目标通过魔法效应恢复HP时，该中毒会提前结束。 \
  为了使用本诡计，你必须携带一套制毒工具在身上。
- **摔绊**：5级起，可以消耗1枚诡计骰，使体型比你大不超过1级的生物陷入倒地状态。
- **撤步**：5级起，可以消耗1枚诡计骰，立刻移动至多等于你速度的一半的距离，并且不会引发借机攻击。
- **恍惚**：14级起，可以消耗2枚诡计骰，使目标的下一回合只能选择下列行为中的一项：移动、执行一个标准动作或执行一个附赠动作。
- **击昏**：14级起，可以消耗6枚诡计骰，迫使目标进行1次强韧豁免。豁免失败则选择令其陷入昏迷或麻痹状态，持续一分钟或受到任何伤害。
- **眩目**：14级起，可以消耗2枚诡计骰，使目标陷入目盲状态，持续至其下个回合结束。
...
##### 1级：审判
*替换【圣武斩】特性* \
你可以使用附赠动作消耗［PB÷2］点MP，对6格内可见的1名敌人降下审判，持续至战斗结束，或目标的HP降至0。 \
...
当你使用自己制造的枪械或弹药时，可以将［DEX-2］(最低0) 加入枪械造成的伤害中，并将重击伤害提升至4D8。9级起，重击伤害提升至6D8；17级起，提升至8D8。
