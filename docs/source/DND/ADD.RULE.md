...
1. 所有属性从10开始。
2. 获得6次**属性提升**，上限为16。
3. 选择A或B：(A) 获得1次**属性提升**；或 (B) 获得2次**属性提升**，再承受1次**属性缺陷**。上限为18。
...
你获得以下增益：
- **强化双持**：进行双持攻击时，主手攻击因此承受的减值减半，副手攻击因此承受的减值降低2点。
...
*通用专长（先决：APR10+，精通双持）* \
你获得以下增益：
- **双重撕裂**：进行双持攻击时，如果一名生物被你命中2次，则可以额外对其造成1D10点相同类型的伤害。
- **双持切割**：如果你拥有【双武器战斗】战斗风格，则可以在进行双持攻击时，将攻击属性调整值加入每一次副手攻击的伤害检定 (若原本不行)。

##### 法术打击
*典范专长（先决：等级4+，SPR1+）* \
1回合1次，当你使用【魔法】动作施展法术时，在施展法术之前或之后，你可以使用附赠动作额外以单手发动1次武器攻击。 \
...
- 该法术的额外伤害调整为1D4，你可以将额外伤害的伤害类型变为与武器相同。5级/11级/17级起，额外伤害提升为1D8/2D6/2D8点。
