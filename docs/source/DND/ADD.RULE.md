...
- **伤害骰尺寸**：伤害骰的尺寸会按以下顺序提升：D4 --> D6 --> D8 --> D10 --> D12。提升伤害骰尺寸的效果不会叠加。
...
- **稳妥打击**：投掷徒手・武器攻击的伤害前，你可以将每枚徒手・武器伤害骰改为［(尺寸+1)÷2］点伤害，小数最后舍去。
...
*起源专长* \
你获得以下增益：
- **快步**：当你使用移动动作时，如果需要消耗的移动力超过1格，则减少1格。
...
- 使用【幽影刃】法术创造的武器发动的攻击，同时视为法术攻击，并且可以使用SPR代替APR决定阶层加值，还可以在攻击・伤害检定中使用施法属性调整值代替力量或敏捷。
...
- 你的空手可以同时视为持用具有「轻型，灵巧」词条的武器，你的徒手打击同时视为武僧武器攻击。
