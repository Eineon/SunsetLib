...
- **移动动作**：意义相当于「消耗等于最大速度的移动力」。你每回合最多只能使用1个移动动作。 \
  执行移动动作不视为进行移动，但无法在速度为0时使用，也会受到与移动相关的限制 (例如**恍惚**诡计)。
- **整轮动作**：意义相当于「使用1个标准动作与1个移动动作」。
- **动作转换**：你可以使用标准・移动动作获得1个额外的附赠动作。如果用于施法，可以使用【魔法】动作转换。你无法以此在1回合内重复使用同一个效果。
- **威能动作**：【威能】是一种新的标准动作选项。
- **属性提升**：选择1项属性+2。如果令属性超过20，则变为+1。
...
### 专长
- 每当人物等级提升至2的倍数时，你都会习得1项满足条件的专长。如果用于习得起源・战斗风格专长，你可以选择1项属性使其初始值+1，最高20。
...
- 原本的专长对移动速度的提升视为内在加值。
- 移除专长：属性值提升、投掷武器战斗、强健身心、中甲大师、重甲大师、双持客、粉碎者、穿刺者、劈砍者
...
##### 盲斗
*战斗风格专长（先决：APR2+，察觉熟练）* \
你获得2格盲视。APR6级起，你会得知6格内对你不具全身掩护的生物占据的方格位置，但不会获知具体位置。 \
APR10级起，距离你6格内的不可见生物对你发动的攻击检定不会因此获得优势。APR14级起，移除此距离限制。
...
##### 战斗专攻
*战斗风格专长（先决：APR2+）* \
选择1道法术，或是1种已熟练的武器，你使用所选项目进行的攻击检定获得2点内在加值。 \
...
火焰刀视为你拥有熟练的「灵巧」简易武器，基础造成［[环阶+1]D6］点火焰伤害，无法提升伤害骰级别。 \
...
幽影刃视为你拥有熟练的「灵巧，轻型，投掷 (射程 4/12)」简易武器，基础造成［[⌈环阶÷2⌉+1]D8］点心灵伤害，无法提升伤害骰级别。 \
你可以通过【攻击】或【魔法】动作使用其发动攻击，这同时视为法术攻击，可以使用SPR代替APR决定等级加值与全力攻击效果，还可以使用施法属性作为攻击属性。 \
...
##### 法剑术
*戏法 塑能（奇械师、吟游诗人、牧师、德鲁伊、术士、魔契师、法师、邪狱使）* \
**施法时间**：附赠动作 \
**施法距离**：自身 \
**法术成分**：V、S、M（一把你熟练的武器） \
**持续时间**：1分钟 \
...
- **翠炎**：额外造成［[X-1]D8］点火焰伤害，你可以将基础伤害的类型改为火焰。 \
  可以选择距离目标X格内你可见的另一名生物作为第二目标，使用本次攻击的判定值挑战第二目标的反射SC，成功则对其造成［[X-1]D8+施法属性调整值］点火焰伤害。
...
你不会再获得其他来源的【（额外攻击:Extra Attack）】及其他类似特性，而是改为获得1项满足条件的战斗风格专长。
...
- **奥能战技**：从“奥能战技项”中习得2项战技。奥能战技使用你的职业DC作为豁免DC。你还可以将奥能战技作为法术准备。
- **奥能卓越骰**：你额外获得［(智力值-9)÷4］(最低1) 枚卓越骰，这些卓越骰只能用于启动奥能战技。
