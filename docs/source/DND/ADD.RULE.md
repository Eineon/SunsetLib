...
- 原本的专长对移动速度的提升视为内在加值。
- 移除专长：属性值提升、箭术、投掷武器战斗、强健身心、中甲大师、重甲大师、双持客、粉碎者、穿刺者、劈砍者
...
每轮你都可以重掷最多［STR］枚该武器的基础伤害骰，并使用新的结果。
...
- **法术反击**：你可以使用要求你发动法术攻击的【魔法】动作进行反击。 \
...
你每轮可以额外发动1次借机攻击。如果你的敏捷17+，则提升至2次。 \
...
- **天生护甲**：激活本效果时，选择2项不同的属性。你可以使用［10+所选属性调整值总和］作为基础AC，最高［17+⌈APR÷4⌉］。
- **自由之盾**：你可以使用持用盾牌的手持握其他物品。若使用该手持握的武器发动攻击，其攻击检定受到2点减值。
...
##### 法术打击
*通用专长（先决：APR5+，SPR2+）* \
每回合1次，你在进行全力攻击时，可以将其中1次攻击替换为执行1个属于敌对行为的【魔法】动作。 \
被替换的那次攻击所承受的全力攻击的减值，也会作用在该【魔法】动作的攻击检定与豁免DC上。 \
若为该【魔法】动作消耗法术位，其环阶不能超过［最大环阶-2］环。 \
...
#### 全力攻击
APR5级后，每回合1次，你在执行【攻击】动作时，可以在发动攻击前使用移动动作进行全力攻击，从以下效果中选择1项发动。
...
- **法术唤回**：以一个附赠动作，你可以消耗2X枚卓越骰，创造1枚X环 (最高不超过你的最大环阶) 的临时法术位，持续至回合结束。 \
...
你与灵光内盟友的基础豁免获得1点状态加值。如果你的魅力17+，则提升至2点。
