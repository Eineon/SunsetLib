...
- **威能动作**：【威能】是一种新的标准动作选项。
- **属性提升**：选择1项属性使其初始值+2。若会令初始值超过20，则变为+1。
...
- **灵活着装**：若该护甲的敏捷上限至少为1，你可以在着装该护甲时降低1点护甲值，增加1点敏捷上限；或是降低1点敏捷上限，增加1点护甲值。
- **着甲移动**：若该护甲为中甲，移除其「嘈杂」词条 (若有)，并添加「柔韧」词条。
- **不落要塞**：若该护甲为重甲，你会在开始自己的回合时，获得可以与其他来源叠加的［1枚熟练骰+STR］点临时生命，持续至你的下个回合开始。
...
- 作为法术效果的一部分而发动的武器攻击，同时视为法术攻击，并且可以使用SPR代替APR决定等级加值。
...
1. 你可以使用施法属性作为攻击属性。
2. 本次攻击同时视为法术攻击，可以使用SPR代替APR决定等级加值。
...
**法术成分**：V、S、M（一片漆树叶） \
**持续时间**：专注，至多10分钟 \
你在一只空手上召出一把火焰刀。当你放下刀刃时其将消失不见，你可以用一个附赠动作重新将其唤出。 \
火焰刀视为拥有「灵巧」词条的简易武器，基础造成［[环阶+1]D6］点火焰伤害，无法提升伤害骰级别。 \
你可以通过【攻击】或【魔法】动作使用其发动攻击，这同时视为法术攻击，可以使用SPR代替APR决定等级加值与全力攻击效果，还可以使用施法属性作为攻击属性。 \
火焰刀会提供2格的明亮光照以及额外2格的微光光照。你可以使用附赠动作缩小或还原火焰刀的光照范围。
...
**法术成分**：V、S \
**持续时间**：专注，至多10分钟 \
你将幽影交织成手中实体化的剑。当你放下或投掷出幽影刃时，它将在回合结束时消失不见，你可以用一个附赠动作重新将其唤出。 \
幽影刃视为拥有「灵巧，轻型，投掷 (射程 4/12)」词条的简易武器，基础造成［[⌈环阶÷2⌉+1]D8］点心灵伤害，无法提升伤害骰级别。 \
你可以通过【攻击】或【魔法】动作使用其发动攻击，这同时视为法术攻击，可以使用SPR代替APR决定等级加值与全力攻击效果，还可以使用施法属性作为攻击属性。 \
使用幽影刃攻击身处微光・黑暗中的生物时，你进行的攻击检定具有优势。
...
- **持续伤害**：记作「**持续(伤害类型)伤害_(伤害值)**」。每当你结束回合时，都会受到列出类型的伤害。 \
  伤害类型不同的持续伤害可以互相叠加。若同时受到多个相同类型的持续伤害，伤害值较高的一方会压制较低的一方。
:::
...
- **猛击**：这次【攻击】的首次武器攻击基础伤害提升［((APR+1)÷6)[W]］点。
...
只要你拥有豁免倾向为强的职业，对应豁免获得的等级加值便会提升2点。
...
当你使用自己制造的枪械或弹药时，可以将［攻击属性调整值-2］(最低0) 加入枪械造成的伤害中，并将重击伤害提升至4D10。9级起，重击伤害提升至6D10；17级起，提升至8D10。
