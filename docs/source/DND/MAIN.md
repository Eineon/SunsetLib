...
- **威能动作**：【威能】是一种新的标准动作选项。
- **即时移动**：记作「**N格即时移动**」，意义相当于「获得N格额外移动力，并立即使用自身拥有的任意移动方式消耗额外移动力进行移动，移动结束时会失去剩余的额外移动力」。即时移动不会触发借机攻击。无法在速度为0时进行即时移动。
...
- **冠名调整值**：拥有名称的调整值不会与同名调整值叠加，包括各项属性的调整值。魔法物品提供的加值默认为增强加值。
...
- 如果在一次属性检定中同时拥有技能与工具的熟练，则会获得1点支援。若有专精，则改为获得1点。

### 属性
各项属性的初始值为6，你可以将合计40点属性值分配在这些属性上，最高18。
...
##### 无实体
拥有【无实体】原型的生物具有力场与心灵之外的伤害抗性，并免疫非魔法伤害与那些需要肉身的效果，例如毒素・重击・偷袭伤害、中毒・侵蚀状态。
...
#### 专注
专注豁免改为单独的一种豁免检定，其豁免调整值为PB加上等于［APR÷2］与［SRP÷2］中较高一方的RB，并使用CON作为关键属性。 \
受到伤害不会再引发专注豁免，改为一次性承受至少［(专注SC-10)×2］(最低1) 点伤害则打断专注。 \
如果处于无法维持专注的状态，你进行的专注豁免将会自动失败。
...
如果DM认为可行，则你可以重新进行1次对抗屈服的豁免检定。如果该效应无需进行豁免，则改为由采取「驱散屈服」的生物合适的技能进行1次对抗施加屈服的效应DC的属性检定。
...
### 整备
整备是一种轻度活动。每次整备必须连续进行至少10分钟，才会计入有效进度。 \
任何能够打断长休的因素均会打断整备，并使你失去整备进度中未满1小时的部分。 \
你可以将一次整备拆分为多次完成。在2次长休之间，你最多只能进行8小时的整备。
...
- **痛击**：这次【攻击】中你选择的1次武器攻击的基础伤害增加［((APR+1)÷6)W］点。
...
#### 法力值
【施法】特性不会再给予你法术位，改为提供［⌈(SPR+基本SKM)×SPR÷2⌉］(最低4) 点MP，不计入无【施法】职业的SPR。 \
...
如果你拥有多个施法职业，则根据以下方式确定最大环阶。你可以准备不超过最大环阶的法术，即便其超过单一职业的最大环阶。
...
##### 1级：擦掠攻击
当你拥有熟练的武器攻击失手时，若判定值比目标值低不超过4点，你依然可以对目标造成［WD枚数+AKM］点擦掠伤害，伤害类型与攻击相同。WD尺寸每比D4大1级，擦掠伤害就会增加［WD枚数÷2］点。 \
...
##### 5级：踏入传奇
以下特性各可以使用1次，完成长休或短休时恢复使用次数。此后每提升6级，各增加1次使用次数。
- **传奇抗性**：当你被攻击命中，或是豁免失败时，可以消耗1次英雄激励使攻击失手，或是豁免成功。无法对被你反击的攻击使用本特性。
- **传奇动作**：每轮1次，当其他生物结束回合时，你可以消耗1次英雄激励发动下列选项之一。
  + **即时行动**：你执行1个标准动作，不能是【魔法】・【预备】动作。
  + **即时移动**：你进行**X格即时移动**，X等于你最大速度的一半 (向上取整)。
...
2. 你可以消耗1次自然纽带创造1枚最高［⌈职业等级÷4⌉］环的法术位，持续至你完成长休。该法术位只能用于施展本职法术。 \
...
##### 12级：至高战技
*新增特性* \
当你投掷先攻时，会获得1枚卓越骰，持续至战斗结束。 \
16级起，每回合1次，当你使用一项战技时，可以不消耗卓越骰，而是掷1D8并使用结果作为代替。 \
20级起，当一名敌人开始回合时，你会获得1个反应，持续至其回合结束。这个反应只能用于发动借机攻击、战技或专长 (不含专长提供的法术)。
...
- **法术唤回**：以一个附赠动作，你可以消耗3X枚卓越骰，创造1枚X环 (最高不超过你的最大环阶) 的法术位，持续至回合结束。 \
...
- **穿梭弹**：你发动远程武器攻击时，可以消耗1枚卓越骰，替代攻击检定，以你为源点影响6格长的线状区域，随后弹药消失。该线状区域能够穿越固体障碍。线状区域内的每名生物必须进行一次反射豁免，失败则受到［1W+AKM+2D］点穿刺伤害。豁免成功则其受半伤。
- **追踪弹**：你发动远程武器攻击时，可以消耗1枚卓越骰，替代攻击检定，选择一名你在前1分钟内见过的生物。弹药向该生物飞去，绕过拐角（如果有必要）并忽视半身掩护和四分之三掩护。若目标身处武器的最大射程内，其必须进行一次反射豁免，失败则受到［1W+AKM+2D］点力场伤害，且你会知晓目标当前的位置。豁免成功则目标仅受半伤。如果目标在你武器的最大射程范围之外，则你的弹药会在射击至尽可能远的距离后消失。
