...
对于前者，该生物可以使用施法属性进行1次专注豁免，对抗你的攻击DC，成功则不会引发借机攻击。 \
对于后者，该生物可以使用敏捷进行1次专注豁免，对抗你的攻击DC，成功则不会引发借机攻击。
...
#### 专注
专注豁免改为单独的一种豁免检定，其豁免调整值为PB加上等于［基本APR÷2］与［基本SRP÷2］中较高一方的RB，并使用CON作为关键属性。 \
...
#### 浴血
你的浴血阈值等于［⌈最大HP÷2⌉］。你会在已损失HP超过浴血阈值期间处于浴血。
...
#### 垂危
你的垂危阈值等于［⌈最大HP×5÷6⌉］。你会在已损失HP超过垂危阈值期间处于垂危，面临遭受**处决**的风险。
...
濒死的持续时间结束后，你的伤势也会得到稳定，并在你的下个回合开始时将已损失HP降至垂危阈值。
...
- **成功**：目标陷入<轻度屈服：失控>状态，强度为［1+(判定值-目标值)÷5］。


### 感官
感官的敏锐程度按由低到高的顺序分为以下3个级别：朦胧、模糊、精确。你不会知晓未被任何感官侦测到的事物是否存在。
- **朦胧感官**：对类人生物而言最常见的朦胧感官是嗅觉，范围通常为2格。只能被朦胧感官侦测到的事物，对你拥有重度遮蔽。
- **模糊感官**：对类人生物而言最常见的模糊感官是听觉，范围通常为6格。只能被模糊感官侦测到的事物，对你拥有轻度遮蔽。
- **精确感官**：对类人生物而言最常见的模糊感官是视觉，范围通常为120格。能够被精确感官侦测到的事物，对你不具遮蔽。
