...
#### 行动值
你不能移动，也无法执行标准・附赠动作。你需要消耗（行动值:Action Points, AP）才能采取这些行为。 \
你的最大AP初始为3点。每当你开始回合时，都会获得等于最大AP的AP，持续至回合结束。 \
你可以在自己的回合，通过以下方式消耗AP换取动作，这无需动作。
| 消耗  | 动作                                        |
| :---: | :------------------------------------------ |
|   1   | 1个附赠动作                                 |
|   2   | 1个标准・移动动作，1个附赠动作              |
|  3+   | 合计［消耗-1］个标准・移动动作，1个附赠动作 |

此外，你也可以消耗［最大AP］(最低1) 点AP来换取一个移动动作。

:::{admonition} 移动动作
:class: seealso
若最大速度不为0，你可以使用移动动作，以下两种移动方式之一进行移动。
- **行进**：你获得等于最大速度的移动力，持续至回合结束。
