...
- **翻面**：意义相当于「将骰值改为［尺寸+1-原值］」。例如，对1D6掷出的2进行翻面，骰值将变为5。
...
##### 命能治疗
拥有【命能治疗】原型的生物会被命能治疗、被虚能伤害。 \
没有【命能治疗】原型的生物免疫虚能效应造成的纯化伤害。

##### 虚能治疗
拥有【虚能治疗】原型的生物会被命能伤害、被虚能治疗。 \
没有【虚能治疗】原型的生物免疫命能效应造成的纯化伤害。

##### 圣洁
拥有【圣洁】原型的生物无法使用邪秽效应，并免疫【圣洁】生物与圣洁效应造成的纯化伤害。

##### 邪秽
拥有【邪秽】原型的生物无法使用圣洁效应，并免疫【邪秽】生物与邪秽效应造成的纯化伤害。

##### 无魂
拥有【无魂】原型的生物无法使用圣洁与邪秽效应，并免疫【圣洁】・【邪秽】生物与圣洁・邪秽效应造成的纯化伤害。
...
你的最大AP初始为3点。最大AP最低为0。你会在最大AP降至0点的期间陷入失能状态。 \
每当你的开始回合时，如果不处于失能状态，将会获得等于最大AP的AP，持续至你的下个回合开始。

你可以在自己的回合，通过以下方式消耗AP换取执行动作的行动权，消耗AP本身无需动作。 \
未使用的行动权会在你的下个回合开始时失效。
| 消耗  | 行动权                                         |
| :---: | :--------------------------------------------- |
|   1   | 1个附赠动作※                                   |
|   2   | 1个标准动作，1个附赠动作                       |
...
*※ 如果你的最大AP为1，可以改为获得1个标准动作*

:::{admonition} 转换行动权
:class: seealso
你随时可以将一个行动权转换为更次一级的行动权，转换本身无需动作。 \
标准动作的行动权可以转换为移动动作，移动动作的行动权可以转换为附赠动作。 \
一个行动权可以被多次转换。例如，你可以转换2次标准动作的行动权，用它来执行一个附赠动作。
:::

:::{admonition} 移动动作
:class: seealso
在最大速度不为0的期间，你可以使用移动动作，采取以下移动方式之一。
...
:::{admonition} 全力动作
:class: seealso
意义相当于消耗1次标准动作与1次移动动作，并且每回合最多只能执行1次。
:::
...
- **阻碍移动**：你的移动动作只能用于进行匍匐。你可以消耗［最大速度÷2］(最低1) 格移动力从倒地状态起身，这会引发借机攻击。 \
  若在移动过程中陷入倒地状态，你会同时失去所有剩余移动力。
...
### 稀有度
在辨识拥有稀有度的项目信息时，检定DC会根据其稀有度获得加值。 \
根据角色的出身与经历，项目的稀有度可能也会发生变化。例如，幽暗地域的怪物对地底居民而言很常见，但对地表居民是罕见。
...
#### 擦掠攻击
你拥有熟练的武器攻击失手时，若判定值比目标值低不超过5点，则依然可以对目标造成［WD枚数］点擦掠伤害，伤害类型与攻击相同。WD尺寸每比4高2点，就将擦掠伤害增加［⌈WD枚数÷2⌉］点。 \
若你可以将属性调整值完整・部分加入武器的伤害检定，则也可以将相同的属性调整值完整・部分加入擦掠伤害。 \
你在反击时无法造成擦掠伤害。若你发动的攻击被成功反击，你也无法造成擦掠伤害。
...
#### 夹击
你与至少1名盟友满足以下所有条件期间，你们会对目标敌人形成夹击，此时目标对抗你们发动的攻击具有**破绽**。
...
#### 翻滚
你可以在自己的回合消耗1格移动力进行翻滚，移动1格距离。你无法在困难地形翻滚，也无法翻滚进入困难地形，因被生物占据而形成的困难地形除外。 \
若你在敌人的威胁范围内翻滚，则需要进行1次特技检定，分别对抗每名敌人的攻击DC，成功则不会因移动而引发对应敌人的借机攻击。 \
当你进行翻滚，并且没有被借机攻击命中时，可以穿越敌人占据的空间。 \
你每进行1次翻滚，在本回合后续进行的所有翻滚都需要额外消耗1格移动力。

#### 垂危
你的垂危阈值等于［最大HP+当前CON-人物等级×3］。你会在损伤超过垂危阈值期间处于垂危，面临遭受**处决**的风险。
...
#### 濒死
当你的HP降为0且没有立即死亡时，将会承受1级**濒死**状态，持续至你完成长休或是死亡。如果你的当前体质21+，濒死的持续时间变为1小时。 \
...
- **失败**：若判定值比目标值低6点或更多，你将失去平衡，陷入倒地状态。
...
- **紧勒**：你可以使用移动动作，以徒手打击对受擒于你的任意名生物造成［1WD］点伤害，目标可以进行1次对抗你的徒手攻击DC的典型强韧豁免。 \
  紧勒不会进行攻击检定，也不会造成那些要求你攻击命中的附加效应。
...
#### 萎靡
萎靡期间，你将遭受以下效应。
- **自我影响**：你进行的D20检定受到1点妨碍。
- **他人影响**：其他生物为对抗你而进行的D20检定获得1点支援。
...
- **自疗**：消耗并投掷最多［PB÷2］枚HD，为每个出目加上CON作为骰值，恢复等于骰值总和的HP。你可以在看到一枚HD的出目后决定是否追加消耗HD。
...
- **勇气矮人**：你对抗恐慌效应的豁免检定具有优势。当你在D20检定中的D20掷出1时，你可以重新掷骰，但你必须使用重骰的结果。
...
- **熊鬼异人**：你可以消耗1点技力以附赠动作执行【疾走】动作，并获得［PB］点内在韧性。 \
  当你的HP至0且没有死亡时，可以立即稳定伤势，随后将损伤降至垂危阈值；使用本效应后，你需要完成长休才能再次使用。
...
歌利亚拥有以下种族特性。
- **大型形态**：你获得【变巨术/缩小术】作为原能根源的本能法术。每天1次，你可以对自己施展此法术制造**变巨**效应，此时将施法时间视为「附赠动作」，并且无需消耗法术位。
...
#### 全力攻击
WPR5级起，作为全力动作，你可以执行1次【攻击】，并从以下效果中选择1项发动。
- **连击**：这次【攻击】能够额外发动1次武器攻击，但其攻击检定受到2点减值。WPR11级/17级起，能够再发动第2次/第3次额外攻击，其攻击检定受到6点/10点减值。
- **痛击**：这次【攻击】中你选择的1次武器攻击的基础伤害增加［[(WPR+1)÷6]WD］点。
...
#### 法力骰
你的法力骰尺寸为4点。SPR1级起每提升4级，将法力骰尺寸增加2点，不计入无【施法】特性职业的SPR。 \
...
| 近战武器 | 熟练  |   伤害   | 词条                          | 精通  | 价格  |
| :------: | :---: | :------: | :---------------------------- | :---: | :---: |
|   短棒   | 简易  | 1D4 钝击 | 轻型                          | 缓速  |   F   |
|   匕首   | 简易  | 1D4 穿刺 | 灵巧，轻型，投掷_4            | 讯击  |   F   |
|   飞镖   | 简易  | 1D4 穿刺 | 灵巧，轻型，投掷_4            | 侵扰  |   F   |
|  铁手套  | 简易  | 1D4 钝击 | 轻型，装具                    | 推离  |   F   |
|   巨棒   | 简易  | 2D4 钝击 | 双手                          | 推离  |   F   |
|   手斧   | 简易  | 1D6 挥砍 | 轻型，投掷_4                  | 侵扰  |   F   |
|   标枪   | 简易  | 1D6 穿刺 | 投掷_6                        | 缓速  |   F   |
|   轻锤   | 简易  | 1D4 钝击 | 轻型，投掷_4                  | 迅击  |   F   |
|  硬头锤  | 简易  | 1D6 钝击 | —                             | 削弱  |   F   |
|   长棍   | 简易  | 1D6 钝击 | 多用_1                        | 失衡  |   F   |
|   镰刀   | 简易  | 1D4 挥砍 | 轻型                          | 讯击  |   F   |
|    矛    | 简易  | 1D6 穿刺 | 多用_1，投掷_4                | 削弱  |   F   |
|   长矛   | 简易  | 2D4 穿刺 | 重型，触及，双手              | 推离  |   F   |
|   战斧   | 军用  | 2D4 挥砍 | 多用_1                        | 失衡  |  1把  |
|   链枷   | 军用  | 2D4 钝击 | —                             | 削弱  |  1把  |
|  长柄刀  | 军用  | 2D5 挥砍 | 重型，触及，双手              | 横扫  |  2把  |
|   巨斧   | 军用  | 2D6 挥砍 | 重型，双手                    | 横扫  |  3把  |
|   巨剑   | 军用  | 2D6 挥砍 | 重型，双手                    | 削弱  |  3把  |
|    戟    | 军用  | 2D5 挥砍 | 重型，触及，双手              | 横扫  |  2把  |
|   骑枪   | 军用  | 2D5 穿刺 | 重型，触及，双手 (骑乘时除外) | 失衡  |  1把  |
|   长剑   | 军用  | 2D4 挥砍 | 多用_1                        | 削弱  |  1把  |
|   巨锤   | 军用  | 2D6 钝击 | 重型，双手                    | 失衡  |  3把  |
|  钉头锤  | 军用  | 2D4 穿刺 | —                             | 削弱  |  1把  |
|   刺剑   | 军用  | 2D4 穿刺 | 灵巧                          | 侵扰  |  2把  |
|   弯刀   | 军用  | 1D6 挥砍 | 灵巧，轻型                    | 迅击  |  2把  |
|   短剑   | 军用  | 1D6 穿刺 | 灵巧，轻型                    | 侵扰  |  1把  |
|  三叉戟  | 军用  | 2D4 穿刺 | 多用_1，投掷_4                | 失衡  |   F   |
|   战镐   | 军用  | 2D4 穿刺 | 多用_1                        | 削弱  |   F   |
|   战锤   | 军用  | 2D4 钝击 | 多用_1                        | 推离  |  1把  |
|    鞭    | 军用  | 1d4 挥砍 | 灵巧，触及                    | 缓速  |   F   |

| 远程武器 | 熟练  |   伤害   | 词条                      | 精通  | 价格  |
| :------: | :---: | :------: | :------------------------ | :---: | :---: |
|   轻弩   | 简易  | 2D4 穿刺 | 弹药_16，装填，双手       | 缓速  |  2把  |
|   短弓   | 简易  | 1D6 穿刺 | 弹药_16，助进，双手       | 侵扰  |  2把  |
|  投石索  | 简易  | 1D4 钝击 | 弹药_6，助进              | 缓速  |   F   |
|  吹箭筒  | 军用  | 1D4 穿刺 | 弹药_6，装填，助进        | 侵扰  |  1把  |
|   手弩   | 军用  | 1D6 穿刺 | 弹药_6，轻型，装填        | 侵扰  |  7把  |
|   重弩   | 军用  | 2D5 穿刺 | 弹药_20，重型，装填，双手 | 推离  |  5把  |
|   长弓   | 军用  | 2D4 穿刺 | 弹药_30，助进，重型，双手 | 缓速  |  5把  |
...
- **重型**：替换原本的效果。如果你的力量值低于13，则无法使用重型武器；如果低于15，则使用重型武器发动的攻击检定具有1点妨碍。 \
  被共持的重型武器在判断上述效果时，将你的力量值视为比实际高2点。
