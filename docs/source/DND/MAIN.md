...
##### 无定型
拥有【无定型】原型的生物免疫那些依赖特定解剖结构的效果，例如武器攻击的重击・偷袭伤害。 \
只要此类生物没有携带或穿着任何东西，就能跻身通过最窄1寸宽的空间。
...
如果攻击检定掷出N20，或是攻击判定值比目标值高出至少10点，还会形成特殊重击，取满基础伤害骰。 \
如果一个效果原本会扩大你的重击范围，增加的每点重击范围，都会改为使重击骰枚数获得2点增强加值。

:::{admonition} 重击豁免
:class: seealso
被重击攻击命中的生物可以尝试进行1次对抗攻击DC的重击豁免。 \
重击豁免没有关键属性，将［重击攻击的目标值-10］作为调整值；如果重击攻击是一次反击，改为以［AC-10］作为调整值。
- **成功**：取消重击，但你依然受到一半重击伤害。形成特殊重击的攻击依然会取满基础伤害骰。
- **失败**：如果掷出N1，或是判定值比目标值低10点或更多，这次攻击还会对你形成特殊重击。
:::
...
- **成功**：你受到一半伤害。若豁免判定值比目标值高10点或更多，你不受任何伤害。
- **失败**：你受到全额伤害。若豁免判定值比目标值低10点或更多，你额外受到［[PB]D8］点相同类型的伤害。
...
#### 夹击
当敌人同时处于你和你盟友的威胁范围内，并且你们分别身处敌人所在空间的对侧或对角时，你们会对敌人形成夹击，此时这名敌人对你们而言拥有破绽状态。 \
为了形成夹击，这名敌人必须对你可见，并且你必须能够与这名盟友交流。
...
- **攻击姿态**：使你的武器攻击检定承受［PB÷2］点减值，令武器攻击的伤害骰级别提升［PB÷2］级。
- **防御姿态**：使你的所有攻击检定承受［PB÷2］点减值，令AC与反射豁免获得［PB÷2］点加值。

##### 1级：擦掠攻击
你拥有熟练的武器攻击失手时，若判定值比目标值低不超过4点，则依然可以对目标造成［WD枚数+AKM］点擦掠伤害，伤害类型与攻击相同。WD尺寸每比D4大1级，擦掠伤害就会增加［WD枚数÷2］点。 \
...
19级起，选择2项属性，再分别选择A或B：(A) 该属性+10，最高20；(B) 该属性+2。
...
##### 5级：毁灭攻击
你可以在自身发动的攻击命中时造成额外伤害，伤害值由攻击类型决定。一名生物每回合最多承受1次此伤害。
- **武器攻击**：造成［⌊(人物等级+1)÷6⌋WD］点相同类型的额外伤害。
- **法术攻击**：造成［⌊(人物等级+1)÷6⌋PD］点相同类型的额外伤害。
- **混合攻击**：如果同时属于武器攻击与法术攻击，你需要从中选择1种决定额外伤害值。
...
- **增强障壁**：每回合1次，你可以在自己的回合获得［2PD+WIS］(最低1) 点增强障壁。 \
