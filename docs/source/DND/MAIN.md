...
- **传奇动作**：每轮1次，当其他生物结束回合时，你可以消耗1次英雄激励发动下列选项之一。 \
  你不能在无法执行反应时执行传奇动作。受到突袭时，你需要在自己的第一个回合结束后才能执行传奇动作。
...
- **强袭**：你可以消耗最多［⌈职业等级÷3⌉］枚诡计骰，直到你的下个回合结束为止，使用精准武器对目标发动攻击时APR获得等量加值。 \
  即使已经应用了其他诡计，你仍然可以在同一次攻击上应用本诡计。
- **淬毒**：5级起，你可以消耗1枚诡计骰，迫使目标进行1次强韧豁免，失败则令目标陷入中毒状态，持续时间如同「谐振」。
- **摔绊**：5级起，你可以消耗1枚诡计骰，迫使目标进行1次反射豁免，失败则令体型比你大不超过1级的生物陷入倒地状态。
- **撤步**：5级起，你可以消耗1枚诡计骰，立刻移动至多等于你速度的一半的距离，并且不会引发借机攻击。
- **恍惚**：14级起，你可以消耗2枚诡计骰，迫使目标进行1次反射豁免，失败则令目标的下一回合只能选择下列行为中的一项：移动、执行一个标准动作或执行一个附赠动作。
- **眩目**：14级起，你可以消耗2枚诡计骰，迫使目标进行1次强韧豁免，失败则令目标陷入目盲状态，持续至其下个回合结束。
- **击昏**：14级起，你可以消耗6枚诡计骰，迫使目标进行1次强韧豁免，失败则选择令其陷入昏迷或麻痹状态，持续一分钟或受到任何伤害。
