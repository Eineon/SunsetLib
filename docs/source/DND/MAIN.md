...
若你在敌人的威胁范围内快步，则需要进行1次特技检定，分别挑战每名敌人的攻击DC，成功则不会因移动而引发对应敌人的借机攻击。 \
你也可以进行相同的特技检定，来快步穿越敌人占据的空间。 \
...
#### 战策
你可以执行【驱策】动作发动1项战策。若无特别说明，你只能选择威胁范围内，对你可见且不具全身掩护的生物作为战策的目标。
...
- **成功**：将目标往任意方向直线推动最多［1+(判定值-目标值)÷5］格。从地面往下方推动每1格都需要消耗2格推动距离。 \
  推动目标时，你可以跟着目标一起移动，这不会引发目标的借机攻击。
- **失败**：如果判定值比目标值低5点或更多，你将失去平衡，陷入倒地状态。
- **碰撞**：作为一种**危害**，推撞结束时，每有1格最大推动距离，被推到固体表面上的生物就会受到1D6点钝击伤害。受到至少1点此伤害的生物还会陷入倒地。
...
##### 虚招
你尝试用误导性动作让对手对真正的攻击毫无防备。你可以使用移动动作发动虚招。 \
进行1次欺瞒检定，对抗目标的被动洞悉。如果以【无心智】生物作为目标，其被动洞悉获得优势。
- **成功**：目标对你的下一次近战攻击视为处于破绽状态，持续1轮。如果判定值比目标值高5点或更多，目标会在相同时间内对你的所有近战攻击视为处于破绽状态。
- **失败**：如果判定值比目标值低5点或更多，你对目标的所有近战攻击视为处于破绽状态，持续1轮。

##### 胁迫
你尝试挫败6格内1名能看见或听见你的敌人的士气。你可以使用移动动作发动胁迫。 \
进行1次威吓检定，对抗目标的被动洞悉。如果以无法理解你的生物作为目标，其被动洞悉获得优势。
- **成功**：目标陷入<轻度屈服：惊惧>状态，惊惧强度为［1+(判定值-目标值)÷5］。
...
- **强袭**：你可以消耗最多［⌈职业等级÷3⌉］枚诡计骰，直到你的下个回合结束为止，使用精准武器对目标发动攻击时APR获得等量加值。 \
  即使已经应用了其他诡计，你仍然可以在同一次攻击上应用本诡计。
- **淬毒**：5级起，你可以消耗X (最低1) 枚诡计骰，使用本次攻击的判定值对抗目标的强韧SC，成功则令目标在X轮内陷入中毒状态。重复施加本诡计将会延长中毒的持续时间。
- **摔绊**：5级起，你可以消耗1枚诡计骰，使用本次攻击的判定值对抗目标的反射SC，成功则令体型比你大不超过1级的生物陷入倒地状态。
- **撤步**：5级起，你可以消耗1枚诡计骰，立刻移动至多等于你速度的一半的距离，并且不会引发借机攻击。
- **恍惚**：14级起，你可以消耗2枚诡计骰，使用本次攻击的判定值对抗目标的反射SC，成功则令目标的下一回合只能选择下列行为中的一项：移动、执行一个标准动作或执行一个附赠动作。
- **眩目**：14级起，你可以消耗2枚诡计骰，使用本次攻击的判定值对抗目标的强韧SC，成功则令目标陷入目盲状态，持续至其下个回合结束。
- **击昏**：14级起，你可以消耗6枚诡计骰，使用本次攻击的判定值对抗目标的强韧SC，成功则选择令其陷入昏迷或麻痹状态，持续一分钟或受到任何伤害。
...
如果这次攻击造成伤害，你可以再对目标发动1次**推撞**或**擒拿**。你可以使用发动这次攻击的武器代替空手发起**擒拿**。
