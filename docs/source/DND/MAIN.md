...
- **动作转换**：你可以使用标准・移动动作获得1个额外的附赠动作。若用于施法，可以使用【魔法】动作转换。你无法以此在1回合内重复使用同一个效果。
- **驱策动作**：【驱策】是一种新的标准动作选项。
...
- **界外体**：界外体至少有一部分本质是主物质位面以外的某个位面的产物。一些生物可能原本属于其他原型，当他们达到更高 (或更低) 层次的精神状态时变成了界外种。
- **苍翠体**：苍翠体是有意识的植物或真菌生物，他们和普通的植物与真菌不一样。
...
你最多可以同时维持3个专注。每多维持1个专注，你的专注检定就会承受5点减值。
...
#### 抗性与易伤
替换原本的抗性与易伤。
- **抗性**：若拥有对特定伤害类型的抗性，你每次承受该类型的伤害值，都会受到［PB÷2×5］点状态减值。
- **易伤**：若拥有对特定伤害类型的易伤，你每次承受该类型的伤害值，都会获得［PB÷2×5］点状态加值。
...
#### 行动力
行动力将决定你在自己的回合能够执行多少动作。你的行动力基础为3级，最低降至0级。 \
若行动力在你自己的回合发生改变，新级别的效应会从你的下个回合开始生效。
| 级别  | 效应                                                         |
| :---: | :----------------------------------------------------------- |
|   0   | 你无法使用标准・附赠动作，也无法移动。                       |
|   1   | 你无法使用标准动作。并且无法在使用附赠动作后移动，反之亦然。 |
|   2   | 你无法在使用标准动作后移动，反之亦然。                       |
|   3   | 你可以正常使用动作并进行移动。                               |
|  4+   | 如同3级，并且你可以额外执行［级别-3］个标准动作。            |
...
若你在敌人的威胁范围内快步，则需要进行1次特技检定，分别对抗每名敌人的攻击DC，成功则不会因移动而引发对应敌人的借机攻击。 \
你也可以进行相同的特技检定，来快步穿越敌人占据的空间。 \
你每进行1次快步，在本回合后续进行的所有快步都需要额外消耗1格移动力。

#### 垂危
你的垂危阈值等于［最大HP-HD枚数×2］。你会在损伤超过垂危阈值期间处于垂危，面临遭受**处决**的风险。
...
失控期间，你将遭受以下效应。
- **强度**：本状态记作「**(类型)失控_(强度)**」。 \
  若拥有不同类型的本状态，将强度加在一起决定效果；若类型相同，强度较低的一方会被压制。
- **检定影响**：你进行的D20检定受到［强度］点内在减值。
- **DC影响**：你的所有DC与AC受到［强度］点内在减值。
...
- **行动力影响**：你的行动力受到［强度］点内在减值。

#### 迅捷
迅捷期间，你将获得以下效应。
- **强度**：本状态记作「**迅捷_(强度)**」。重复承受本状态时，强度较低的一方会被压制。
- **行动力影响**：你的行动力获得［强度］点内在加值。 \
  许多给予迅捷状态的效果都会列出你可用额外动作进行的动作类型。若多个效应使你获得迅捷状态，额外动作能进行的动作类型从所有给予迅捷状态的效应列表中选择。
...
- **疲劳**：记作「**疲劳_(强度)**」。每有1级疲劳，你的最大HP就会减损［PB］点。 \
...
### 命刻
每当一个具有明确叙事闭环的场景结束时，DM可以宣布推进1次命刻。例如，解决一场谜题、完成一次潜入、击败一波敌人、成功交涉获得关键信息 \
命刻每次被推进时，所有PC都可以从以下效果中选择1项。
- **回火**：如同完成短休般恢复一项特性的次数・点数。
- **自疗**：恢复［体质值÷2+PB×5］点HP。
...
##### 1级：擦掠攻击
你拥有熟练的武器攻击失手时，若判定值比目标值低不超过4点，则依然可以对目标造成［WD枚数+AKM］点擦掠伤害，伤害类型与攻击相同。WD**尺寸**每比D4大1级，擦掠伤害就会增加［WD枚数÷2］点。 \
...
- **法术攻击**：造成［2⌊(人物等级+1)÷6⌋PD］点相同类型的额外伤害。
...
- **增强障壁**：每回合1次，你可以在自己的回合获得［2PD+SKM］(最低1) 点增强障壁。 \
