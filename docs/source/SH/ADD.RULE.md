...
- **级别>0**：
  ...
  3. 若使用固定值作为骰值，则不掷骰，而是获得［级别×2］(最高8) 点加值。
- **级别<0**：
  1. 在得出骰值后，还会承受［|级别|］枚1D4的减骰。
  2. 若低于-4级，则只保留出目最高的4枚减骰。
  3. 若使用固定值作为骰值，则不掷骰，而是承受［|级别|×2］(最高8) 点减值。
...
### 准备与展现技艺
游戏开始前，或是在间章中，角色可以准备任意项技艺，但这些技艺的**负荷**总和不能高于［灵识-4］L。 \
