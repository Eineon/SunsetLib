document.addEventListener('DOMContentLoaded', function () {
  ...
  const tagData = {
    ...
    '近战': '这种武器只能用来发动近战攻击。',
    '远程': '这种武器只能用来发动远程攻击。',
    '投掷': '这种武器既可以用来发动近战武器，也可以用来发动远程攻击。',
    '轻型': '如果角色在自己的回合仅使用轻型武器发动攻击，则在其回合结束时，每次投掷先攻骰掷出的点数都会获得1点内在减值。',
    '重型': '如果角色在自己的回合使用重型武器发动攻击，则在其回合结束时，每次投掷先攻骰掷出的点数都会受到1点内在加值。',
    '装填': '这种武器每轮只能使用1次。',
    '————': '——————————————————————————————————————————————————',
  ...
  const efctData = {
    ...
    '————': '——————————————————————————————————————————————————',
    'BS': 'BS会令角色在特定的能力值上承受等于效果值的状态减值。<br>针对不同能力值的BS视为不同种类的处境。',
    ...
    '————': '——————————————————————————————————————————————————',
    'GS': 'GS会令角色在特定的能力值上获得等于效果值的状态加值。<br>针对不同能力值的GS视为不同种类的处境。',
